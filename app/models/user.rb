class User < ActiveRecord::Base
  ...
  def friends_in_city city
    array = self.connections.where("blocked = ?", false).public_avail.select { |friend| friend.city_id == city.id }
    array.delete(self)
    return array.uniq
  end
  ...
  def friends_by_city
    ...
    friends = self.connections.where("blocked = ?", false).public_avail.delete_if {|user| user == self }
    friends.each do |f|
